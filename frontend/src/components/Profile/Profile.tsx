    ...
    const fetchUserPosts = async (): Promise<void> => {
      try {
        const response = await axios.get(
          `http://localhost:8019/user-posts/${userInfo!}`
        );
    ...
    <div className="profile">
            ...
            <div key={post._id}>
              ...
              {userInfo === post.author && (
                <div className="#edit-post-container">
                  <Link to={`/edit/${post._id}`} id="edit-post">
                    Edit Post
                  </Link>
                </div>
              )}
              ...
              <img
                src={`http://localhost:8019/${post.cover}`}
                id="profile-post-img"
              ></img>
      ...
      {showCatAndPost && (
        <>
          ...
          <p id="no-posts">
            You currently have no posts. Once you create a&nbsp;
            <Link to="/post" id="post">
              post{" "}
            </Link>
            , it will appear here.
          </p>
